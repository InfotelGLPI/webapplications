...
xgettext --files-from=php_files.list \
  --copyright-holder='Webapplications Development Team' \
  --package-name='Webapplications plugin' \
  -o locales/glpi.pot \
  -L PHP \
  --add-comments=TRANS \
  --from-code=UTF-8 \
...
do
    ...
    #    "{% set labels = {'k': __('Label', 'webapplications')} %}" stays plain text for xgettext
