...
class PluginWebapplicationsPdf extends Fpdf\Fpdf
{
    ...
    public function __construct($title, $subtitle, $id)
    {
        ...
        $this->config = new PluginWebapplicationsConfig();
        $this->config->getFromDB(1);
        $this->webappAppliance = new PluginWebapplicationsAppliance();
    ...
    public function Header()
    {
        ...
        $this->CellTitleValue($this->page_width, 5, Toolbox::decodeFromUtf8(Toolbox::stripslashes_deep($title)), '', 'R', '', 0, $this->title_size - 6, 'black');
        $this->SetY($this->GetY() + 5);
        $this->SetX($this->margin_left);

        if ($this->headernumber > 1) {
            $this->SetY($this->GetY() + 1);
            $this->CellTitleValue($this->page_width, 9, Toolbox::decodeFromUtf8(Toolbox::stripslashes_deep($subtitle)) . ' (' . __('page', 'webapplications') . $this->headernumber . ')', 'TLRB', 'C', 'hardgrey', 0, $this->title_size - 2, 'black');
    ...
    private function firstpage() {
        ...
        $this->MultiCell($this->page_width - ($this->page_width/4), $this->title_height, __('Project leader', 'webapplications') . ' : ' .  Toolbox::decodeFromUtf8(User::getFriendlyNameById($this->appliance->fields['users_id_tech'])) , 'BRT', 'C', '', 0, '', 'black');
        $this->setX($this->margin_left + ($this->page_width /4));
        $this->MultiCell($this->page_width - ($this->page_width/4), $this->title_height, __('Project team', 'webapplications') . ' : ' . Group::getFriendlyNameById($this->appliance->fields['groups_id_tech']) , 'BR', 'C', '', 0, '', 'black');
        ...
        foreach ($contractItemDatas as $contractItemData) {
            ...
            $this->Cell(($this->page_width / 9), $yligne4 - $yligne3, Toolbox::decodeFromUtf8(htmlspecialchars_decode($contract->fields['duration'] . __(' months', 'webapplications'))), '', 1, 'C', false);
    ...
    private function secondpage() {
        ...
        $this->adddataonfourcolomn(__('Associable to a ticket', 'webapplications'),$answer, __('Location', 'webapplications'),$location->fields['name'] ?? '');
        ...
        $group = new Group();
        $group->getFromDB($this->appliance->fields['groups_id_tech']);
        $this->adddataonfourcolomn(Manufacturer::getTypeName(1), $manufacturer->fields['name'] ?? '', __('Group in charge'), $group->fields['name'] ?? '');
        ...
        $this->adddataonfourcolomn(User::getTypeName(1), $user->fields['name'] ?? '', Group::getTypeName(1), $group->fields['name'] ?? '');
        $user = new User();
        $user->getFromDB($this->appliance->fields['users_id']);
        $group = new Group();
        $group->getFromDB($this->appliance->fields['groups_id']);
    ...
    private function thirdpage() {
        ...
        $this->MultiCell(($this->page_width/2) -1, $this->title_height, Toolbox::decodeFromUtf8(htmlspecialchars_decode(_n('Process', 'Processes', 1, 'webapplications'))), 'TLRB', 'C', true, 0, '', 'black');
        ...
        if ($this->GetY() < $yligne4) {
            $this->MultiCell(($this->page_width / 2) - 1, $yligne4 - $this->GetY() + 1, '', 'LRB', 'C', false, 0, '', 'black');
            $this->setXY($this->margin_left, $yligne4);
            $this->MultiCell(($this->page_width / 2) - 1, 1, '', 'LRB', 'C', false, 0, '', 'black');
        } elseif ($this->GetY() > $yligne4) {
            $yligne5 = $this->GetY();
            $this->setXY($this->margin_left, $yligne4);
            $this->MultiCell(($this->page_width / 2) - 1, $yligne5 - $yligne4 + 1, '', 'LRB', 'C', false, 0, '', 'black');
        ...
        $this->setXY($this->margin_left, $this->GetY() + 2);
        $yligne3 = $this->GetY();
        $this->MultiCell(($this->page_width/2) -1, $this->title_height, Toolbox::decodeFromUtf8(htmlspecialchars_decode(__('Physical Infrastructure', 'webapplications'))) , 'TLRB', 'C', true, 0, '', 'black');
        $this->setXY($this->margin_left + ($this->page_width/2) + 1, $yligne3);
        $this->MultiCell(($this->page_width/2) -1, $this->title_height, Toolbox::decodeFromUtf8(htmlspecialchars_decode(\DatabaseInstance::getTypeName(2))) , 'TLRB', 'C', true, 0, '', 'black');
        ...
        foreach (["Computer", "Printer", "Phone", "NetworkEquipment"] as $itemtype) {
            ...
            foreach ($physicalinfraDatas as $physicalinfraData) {
                ...
                $this->MultiCell(($this->page_width/2) -1, 7, Toolbox::decodeFromUtf8(htmlspecialchars_decode($item->fields['name'])) , 'LR',  'C', false);
                $this->setXY($this->margin_left,$this->GetY());
            }
        }
        ...
        foreach ($databasesInstanceDatas as $databasesInstanceData) {
            ...
            $this->MultiCell(($this->page_width/2) -1, 7, Toolbox::decodeFromUtf8(htmlspecialchars_decode($databaseInstance->fields['name'])) , 'LR',  'C', false);
            $this->setXY($this->margin_left + ($this->page_width/2) +1,$this->GetY());
        }
        if ($this->GetY() < $yligne4) {
            $this->MultiCell(($this->page_width / 2) - 1, $yligne4 - $this->GetY() + 1, '', 'LRB', 'C', false, 0, '', 'black');
        ...
        $this->setXY($this->margin_left, $this->GetY() + 2);
        $yligne3 = $this->GetY();
        $this->MultiCell(($this->page_width/2) -1, $this->title_height, Toolbox::decodeFromUtf8(htmlspecialchars_decode(_n("Certificate", 'Certificates', 2))) , 'TLRB', 'C', true, 0, '', 'black');
        $this->setXY($this->margin_left + ($this->page_width/2) + 1, $yligne3);
        $this->MultiCell(($this->page_width/2) -1, $this->title_height, Toolbox::decodeFromUtf8(htmlspecialchars_decode(_n('Stream', 'Streams', 2, 'webapplications'))) , 'TLRB', 'C', true, 0, '', 'black');
        ...
        foreach ($certificatItemDatas as $certificatItemData) {
            ...
            $this->MultiCell(($this->page_width/2) -1, 7, Toolbox::decodeFromUtf8(htmlspecialchars_decode($certificat->fields['name'])) , 'LR',  'C', false);
            $this->setXY($this->margin_left,$this->GetY());
        }
